...
> "⚠️ Per-group files skipped — catalog not available. Run `./install.sh vendor <git-root>` and re-run /scout."
...
Scan for files that contain the marker `<!-- generated by /scout`:
...
Files **without** the `<!-- generated by /scout` marker are user-created — never touch them.
...
- If content > 4,000 chars → split into numbered files (`<group>-1.instructions.md`, `<group>-2.instructions.md`, …), each ≤ 4,000 chars. Split at principle-line boundaries (never mid-line). Each split file gets its own complete frontmatter header and `<!-- generated by /scout` marker.
