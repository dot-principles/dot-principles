...
- **`/dot-prime` requires `/dot-scout`** — in normal mode, prime now checks `install.cfg` for the `scout` marker and stops with a helpful message if scout has not been run. Explicit-mode (`@group` / bare IDs) bypasses this guard.
