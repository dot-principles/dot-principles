...
- **Public docs site scaffold** - added `package.json`, `package-lock.json`, `.vitepress/config.mts`, and `.github/workflows/deploy.yml` to build and publish a VitePress-based GitHub Pages site from this repository.
- **Public docs content layer** - added `content/` as the VitePress source tree with visitor-facing pages for `why`, `getting started`, `how it works`, `commands`, `extending`, and `examples`, plus `README.md` / `INDEX.md` navigation files.
- **Docs architecture records** - added `docs/README.md`, `docs/authoring.md`, `docs/INDEX.md`, and ADR `docs/decisions/0001-public-docs-site.md` to document ownership boundaries, authoring rules, and the in-repo docs architecture.

**Changed**

- **Markdown punctuation normalized repo-wide** - replaced long dash characters in project Markdown source files with plain ASCII hyphens for a less synthetic-looking reading experience.
- **`content/INDEX.md` punctuation normalized** - replaced em dashes in the public docs index with plain ASCII hyphens so the page reads less machine-generated.
- **`README.md` repositioned as the GitHub front door** - it now serves as a shorter GitHub-native entry point that routes readers into the guided docs journey while keeping deep references in `INSTALL.md`, `DESIGN.md`, and `demo/presentation.md`.
- **`DESIGN.md` now records the public docs layer** - Section 1 documents the VitePress presentation layer, the `content/` vs `docs/` split, and the canonical-source boundaries for public docs versus deep reference docs.
- **`demo/presentation.md` now acknowledges the guided docs path** - the demo remains the full workflow walkthrough, but now points evaluators to the shorter onboarding path in `content/` first.
- **VitePress npm scripts now pin the repo root explicitly** - `docs:dev`, `docs:build`, and `docs:preview` now call `vitepress ... .` so extra CLI flags like `--host` and `--port` are parsed as options instead of being mistaken for a site root and producing a dev-server 404.

## [v0.13.0] - 2026-04-29

**Added**

- **`dot-audit` Phase 9 - Re-run audit option** - the Phase 9 commit gate now offers a new option `0. Re-run audit` when the audit found at least one Medium or higher finding. Choosing it jumps back to Phase 5 (Pre-Scan) using the same already-resolved target and principles, re-runs Phases 5-7, then re-enters the Phase 8 fix gate. This surfaces issues that were hidden by the more prominent findings in the previous pass. The fix branch from Phase 8 Step 1 is reused; the pass number increments and is included in the eventual commit message. The option is deliberately not shown for low-only audits to avoid noise.
- **`dot-audit` - workflow diagram** - a Mermaid flowchart is now embedded near the top of `commands/dot/audit.md`, before Phase 1. It shows the full 10-phase flow including all three gated decisions (Fix / Commit / PR) and the conditional re-audit loop from Phase 9 back to Phase 5.
- **`dot-scout` Phase 4 - Exclusion Density Analysis** - before writing any `.principles` files, dot-scout now checks every parent-level proposed entry (groups and bare IDs) against the detected child-directory profiles. If more than 50% of applicable children would need to exclude the entry, it is *demoted*: removed from the parent proposal and added only to the children that actually benefit. For individual principles that >50% of children would suppress via `!PRINCIPLE-ID`, the exclusion is *consolidated* to the parent level (one line instead of many). Both demotion and consolidation are shown in the proposal output with `⬇` and `↑` prefixes and the exclusion ratio. This avoids `.principles` files accumulating large blocks of `!exclusions` purely to suppress inherited rules. The previous Phase 4 (Check Existing), Phase 5 (Write Files), Phase 6 (Emit active.md), and Phase 7 (Emit AI Review Integration) shift to Phases 5-8 accordingly.

**Changed**

- **Version stamped in all generated file headers** - `.principles` files written by `dot-scout` now include `# Generated by dot-scout vVERSION` (was `# Generated by /dot-scout` - the slash was wrong and the version was missing). Installed `SKILL.md` and `.claude/commands/` files now include the `.principles` version in their `generated-by:` frontmatter field (`generated-by: ".principles vVERSION"`).
...
- **`.agents/principles-catalog/` replaces `.principles-catalog/`** - the vendor catalog is now co-located with skills under `.agents/`. Old location read as a fallback for backwards compatibility; uninstaller removes both.
- **`{{PRINCIPLES_DIRECTORY}}` resolves to `.agents/principles-catalog`** - the placeholder substituted at install time changed from `.principles-catalog` to `.agents/principles-catalog`.
...
- **`templates/copilot-cli/`** - Copilot CLI reads `.agents/skills/` natively; no wrapper needed.
- **`templates/copilot-ide/`** - Copilot IDE reads `.agents/skills/` natively (exposed as `/skills:<slug>`); `.github/prompts/` wrappers created duplicate entries with no added value.
- **Per-tool installer subcommands** - `claude`, `copilot`, `copilot-cli`, `copilot-ide`, `codex`, `all` removed. Use interactive mode (`./install.sh <dir>`) to configure wrappers; use `vendor` for CI/scripted syncing.
